...
Stores:
  ...
  - Persistent messaging outbox queue
  - Generic counters for observability
"""
...
def enforce_db_limits(max_db_mb: int = 128, max_chat_rows: int = 200000):
    """Best-effort DB growth control by rows and size."""
    conn = _connect()
    with _DB_LOCK:
        ...
        # Keep queue tables bounded in pathological cases.
        ...
        try:
            if os.path.exists(db_path()):
                ...
                if os.path.getsize(db_path()) > max_bytes:
                    # drop oldest 10% rows and vacuum
                    conn.execute(
...
def prune_delivery_state(ttl_seconds: int = 7 * 24 * 3600):
    conn = _connect()
    cutoff = time.time() - max(3600, int(ttl_seconds))
    with _DB_LOCK:
        conn.execute("DELETE FROM delivery_state WHERE updated_at < ?", (cutoff,))
        conn.commit()
