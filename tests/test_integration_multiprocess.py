...
def test_multiprocess_text_file_and_restart_recovery():
    ...
    with tempfile.TemporaryDirectory(prefix="meshlink_mp_") as td:
        ...
        try:
            ...
            # Pair qa with qb
            r_pair = _rpc(qa_cmd, qa_resp, seq, "pair_with_seed", peer_id="nodeb", seed="ABCDEF", timeout=10.0)
            seq += 1
            assert r_pair.get("ok") is True
            r_pair2 = _rpc(qb_cmd, qb_resp, seq, "pair_with_seed", peer_id="nodea", seed="ABCDEF", timeout=10.0)
            seq += 1
            assert r_pair2.get("ok") is True
